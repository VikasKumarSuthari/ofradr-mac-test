    ...
    unsafe {
        ...
        let _tap = CGEventTap::new(
            ...
            |_proxy, event_type, event| {
                if event_type != CGEventType::KeyDown { return Some(event.clone()); }
                ...
                let typed = event.get_string_value_field(CGEventField::KeyboardEventUnicodeString).unwrap_or_default();
                unsafe {
                    let tf = TEXT_FIELD.load(Ordering::SeqCst);
                    if !tf.is_null() {
                        ...
                        if key_code == kVK_Delete {
                            ...
                            let ns_char = NSString::alloc(nil).init_str(&typed);
